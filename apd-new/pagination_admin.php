  ...
  include("database/db_conection.php");
   ...
   if(isset($_POST['fIndentPage'])){
    ...
    $limit=10;
     ...
     if($_POST['indent_type']==='adRaiPrIndent'){
     ...
     if($_POST['indent_type']==='adRaiFabIndent')
     {
      $result4=mysqli_query($dbcon,"SELECT i.Pre_Id,i.Description, i.Units, i.State,i.Name,i.Raised_Date, i.Approved_Date, i.Patient_Id,i.User_Type,i.Camp_Name,i.TechName,p.Patient_Name FROM prefab_indent_table i,
          patient_details p WHERE i.Patient_Id=p.Patient_Id and i.State='Raised' LIMIT $start_from,$limit");
     ...
     if($_POST['indent_type']==='adRaiRwIndent')
     {
      $resultRaiRw=mysqli_query($dbcon,"SELECT i.R_Indent_Id,i.Description,i.Raised_Date,i.State,i.Approved_Date,i.User_Type 
                            FROM  raw_indent_table i WHERE i.State='Raised' LIMIT $start_from,$limit");
         if(mysqli_num_rows($resultRaiRw) > 0){
          $i=1;
           $resultArrayRaiRw=array();
         while($rowRaiRw = mysqli_fetch_array($resultRaiRw)){

          $resultArrayRaiRw[$i] = array($rowRaiRw['Description'],$rowRaiRw['Raised_Date'],$rowRaiRw['R_Indent_Id']);
           $i++;
          }
          echo json_encode($resultArrayRaiRw);
        }
     }

      if($_POST['indent_type']==='adAppPrIndent')
      {
      ...
      if($_POST['indent_type']==='adAppFabIndent')
      {
     ...
     if($_POST['indent_type']==='adAppRwIndent')
     {
      $resultAppRw=mysqli_query($dbcon,"SELECT i.R_Indent_Id,i.Description,i.Raised_Date,i.State,i.Approved_Date,i.User_Type 
                            FROM  raw_indent_table i WHERE i.State='Approved' LIMIT $start_from,$limit");
         if(mysqli_num_rows($resultAppRw) > 0){
          $i=1;
           $resultArrayAppRw=array();
         while($rowAppRw = mysqli_fetch_array($resultAppRw)){

          $resultArrayAppRw[$i] = array($rowAppRw['Description'],$rowAppRw['Raised_Date'],$rowAppRw['Approved_Date'],$rowAppRw['R_Indent_Id']);
           $i++;
          }
          echo json_encode($resultArrayAppRw);
        }
     }

      if($_POST['indent_type']==='adRejPrIndent')
      {
      ...
      if($_POST['indent_type']==='adRejFabIndent')
      {
